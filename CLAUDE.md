...
- **Runs on**: a Windows laptop, reached from phones on the same Wi-Fi.
...
└── realtime/
    ...
    └── validation.py       username rules and "is this name taken". Pure.
```
...
├── components/             NameInput, UserList, Health
...
takes the `user_id` the route knows and ignores any id inside the payload.
...
- **The two validation files mirror each other**: `Backend/realtime/validation.py` and
  ...
  real check; the frontend only gives instant feedback.
...
- **Windows and the LAN**: the backend needs `--host 0.0.0.0`, Vite needs `--host`,
  and Windows Firewall must allow Python and Node on the current network profile. The
  laptop's address changes, so check with `Get-NetIPAddress` rather than assuming.
...
1. **A throwaway server on port 8799** with script clients, so the owner's own server
   ...
   ```bash
   .venv/Scripts/python.exe -m uvicorn main:app --host 127.0.0.1 --port 8799
   ```
